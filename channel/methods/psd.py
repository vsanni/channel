...
def psd(self, df = None, Window_type = "hanning", a = 0.5, detrend = 1, name = None ):
    ...
    if df is None:
        ...
        raise ValueError("psd: error, window length, %g s, allows a minimum frequency bin of %g " % (Dt, df_min))
    ...
    f, P, S.Windows, S.NormFactor = sp.PSD(self.X, S.w, S.a, S.sf, S.detrend, None, 0)
    ...
    ch = self.__class__((f[0], f[1]-f[0]), P,
                        ...
                        descriptions   = ("frequency", "Power spectral density of %s" %  self.description),
                        parameters     = S
                        )
    #TODO, this is a patch, units conversion 
    ch.unitss = ("Hz" if self.xunits == "s" else "1/(%s)" % self.xunits,
                 "(%s)^2/Hz" % self.units if self.xunits == "s" else "(%s)^2/(%s)" % ( self.units, self.xunits))

    return ch


add_channel_method("psd", {"ChannelCS": psd})
